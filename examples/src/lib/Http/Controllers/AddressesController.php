...
use App\Http\Controllers\ViewModels\AddressViewModel;
use App\DataTransfertObject\AddressDto;
...
final class AddressesController
{
	 ...
	 * @return self
	 */
	public function __construct(Validator $validator, IActionResponseHandler $response, AddressService $service = null)
	{
		# code...
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new AddressDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new Address, $request);;
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new AddressDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		try {
			...
			$viewModel_ = (new AddressViewModel)->setUser($request->user())->set($request->all())->files($request->allFiles());
