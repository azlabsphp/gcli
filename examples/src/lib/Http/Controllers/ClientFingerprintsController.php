...
use App\Services\ClientFingerprintService;
use App\Models\ClientFingerprint;
use App\Http\Controllers\ViewModels\ClientFingerprintViewModel;
use App\DataTransfertObject\ClientFingerprintDto;
...
final class ClientFingerprintsController
{

	/**
	 * Injected instance of MVC service
	 * 
	 * @var ClientFingerprintService
	 */
	private $service;
	 ...
	 * @param ClientFingerprintService $service
	 *
	 * @return self
	 */
	public function __construct(Validator $validator, IActionResponseHandler $response, ClientFingerprintService $service = null)
	{
		# code...
	 ...
	 * @Route /GET /clientfingerprints[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new ClientFingerprintDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new ClientFingerprint, $request);;
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new ClientFingerprintDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		try {
			...
			$viewModel_ = (new ClientFingerprintViewModel)->setUser($request->user())->set($request->all())->files($request->allFiles());
	 ...
	 * @Route /PUT /clientfingerprints/{id}
	 * @Route /PATCH /clientfingerprints/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /clientfingerprints/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
