...
use App\Services\ClientFaceService;
use App\Models\ClientFace;
use App\Http\Controllers\ViewModels\ClientFaceViewModel;
use App\DataTransfertObject\ClientFaceDto;
...
final class ClientFacesController
{

	/**
	 * Injected instance of MVC service
	 * 
	 * @var ClientFaceService
	 */
	private $service;
	 ...
	 * @param ClientFaceService $service
	 *
	 * @return self
	 */
	public function __construct(Validator $validator, IActionResponseHandler $response, ClientFaceService $service = null)
	{
		# code...
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new ClientFaceDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new ClientFace, $request);;
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new ClientFaceDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		try {
			...
			$viewModel_ = (new ClientFaceViewModel)->setUser($request->user())->set($request->all())->files($request->allFiles());
	 ...
	 * @Route /PUT /clientfaces/{id}
	 * @Route /PATCH /clientfaces/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
