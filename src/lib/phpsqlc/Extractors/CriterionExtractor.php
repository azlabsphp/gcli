...
use Drewlabs\PHPSQLC\Utils\CriterionTypes;
...
{
    /** @var bool */
    private $negationOn = false;

    /** @var bool */
    private $handleOuterNegation = false;
    ...
    /**
     * criterion extractor constructor builder
     * 
     * @param bool $group 
     * @param array $aggregators 
     * @return void 
     */
    public function __construct(bool $group, array $aggregators = [])
    {
        $this->group = $group;
    ...
    /**
     * load criterias in the 3rd argument of the function
     * 
     * @param mixed $value 
     * @param array &$parts 
     * @param mixed $context 
     * @param bool $handleOuterNegation 
     * @return void 
     */
    public function getCriteria($value, &$parts = [], $context = CriterionContext::WHERE, $handleOuterNegation = true)
    {
        $currIndex = 0;
        $logicalOperator = 'and';
        ...
        foreach ($value as $index => $val) {
            ...
            if (in_array($val['expr_type'], array('operator', 'reserved'))) { // reserved since k+10 in (in is considered reserved)
                $sep = Expression::lowercase($val['base_expr']);
                if (Expression::isLogicalOperator($sep)) {
                    $logicalOperator = Expression::lowercase($val['base_expr']);
                    continue;
                }

                switch (Expression::lowercase($sep)) {
                ...
                if ($val['sub_tree'] !== false) { // skip cases such ()
                    ...
                    $this->getCriteria($val['sub_tree'], $local, $context, false); // recursion
                    if (!empty($local)) {
                ...
                if (Expression::lowercase($val['base_expr']) == 'against') {
                    $resField = $this->getLeft($index, $value);
                    $resVal = $this->getRight($index, $value, $currIndex, $context);
                    ...
                    $fn = Expression::lowercase($val['base_expr']);

                    if (in_array($fn, $this->aggregators)) {
                        ...
                        $sep = Expression::lowercase($value[$currIndex]['base_expr']);
                        $resVal = $this->getRight($index, $value, $currIndex, $context);
    ...
    private function getArrayParts($val)
    {
        ...
        foreach ($val as $v) {
            if ($v['expr_type'] == 'operator') {
                if (Expression::lowercase($v['base_expr']) == 'not' && (count($fields) - count($values)) == 0) { // not x > 1
                    return false;
                }
                ...
                if (Expression::lowercase($v['base_expr']) != 'and') // prevent group on [or] operator, also in any operation too such as field1+field2 > number (this needs not to be escaped, therefore it will be as separate row)
                    return false;
            } else {
                if ($next == 'field') {
    ...
    public function getLeft(int $index, array $value, $context = CriterionContext::WHERE)
    {
        ...
        $leftOperator = '';
        $isRaw = false;
        while (!Expression::isLogicalOperator($leftOperator)) {
            if ($leftIndex > 0) {
                $leftIndex--;
                $op_type = Expression::lowercase($value[$leftIndex]['expr_type']);
                if ($op_type == 'operator') {
                    if (!$this->isArithmeticOperator(Expression::lowercase($value[$leftIndex]['base_expr']))) {
                        $leftOperator = Expression::lowercase($value[$leftIndex]['base_expr']);
                    } else {
    ...
    public function getRight($index, $value, &$currIndex, $context = CriterionContext::WHERE)
    {
        ...
        while (!Expression::isLogicalOperator($rightOperator)) { // x > 2 and (until you find first logical operator keep looping)
            $rightIndex++;
            if ($rightIndex < count($value)) {

                if (Expression::lowercase($value[$rightIndex]['expr_type']) == 'operator') {

                    if (!$this->isArithmeticOperator($value[$rightIndex]['base_expr'])) {
                        $rightOperator = Expression::lowercase($value[$rightIndex]['base_expr']);
                    } else {
                    ...
                    $context == CriterionContext::HAVING
        ...
        while ($log_operator_count != 2) { // between x and y and (until you find second logical operator keep looping)
            ...
            if ($rightIndex < count($value)) {
                if (Expression::lowercase($value[$rightIndex]['expr_type']) == 'operator') {
                    ...
                    if (Expression::isLogicalOperator($rightOperator)) {
                        $log_operator_count++;
                        $rightOperator = '';
                        $final[] = $valuesArray;
                        $raws[] = $isRaw;
