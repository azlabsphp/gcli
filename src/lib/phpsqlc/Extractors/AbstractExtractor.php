...
{
    protected function hasAlias(array $val)
    {
        return isset($val['alias']) && $val['alias'] !== false;
    }

    protected function getFnParams(array $val, string &$params): string
    {
    ...
    protected function isSingleTable(array $parsed) // duplicate?
    {
        return (isset($parsed['FROM']) && count($parsed['FROM']) == 1 && $parsed['FROM'][0]['expr_type'] == 'table');
    }

    protected function validCountTable(array $parsed)
    {
        unset($parsed['SELECT']);
        unset($parsed['FROM']);
        // if has any other clause (e.g WHERE/HAVING/LIMIT) except those that were removed then it is not valid
        return count($parsed) === 0;
    }
    protected function validJoin(string $join_type)
    {
        return Expression::lowercase($join_type) != 'natural';
    }

    protected function isArithmeticOperator(string $op)
    {
    ...
    protected function isComparisonOperator($operator, $append = [])
    {
        ...
        return in_array(Expression::lowercase($operator), $simple_operators);
    }

    protected function getExpressionParts(array $value, ?array &$parts = null, ?array &$raws = null, bool $recursive = false)
    {
        $valLen = count($value);
    ...
    public function mergeExpressionParts(array $parts)
    {
        return (implode('', $parts));
    }

    protected function getWithAlias(array $val, bool &$isRaw)
    {
    ...
    public function isRaw(array $val)
    {
