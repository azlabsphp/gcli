...
class CriterionBuilder extends AbstractBuilder implements Builder
{
    ...
    public function build(array $parts, array &$skipBag = array()): string
    {
        $queryVal = '';
            ...
            switch ($part['type']) {
                ...
                case CriterionTypes::LIKE:
                    ...
                    if ($part['raw_field'] || $part['raw_value']) {
                        ...
                        if ($part['raw_field'] && !$part['raw_value'] && $this->getValue($part['value']) !== 'null')
                            $inner = Expression::quote($part['field'] . ' ' . strtoupper($op) . ' ? ') . ', ' . '[' . $this->wrapValue($part['value']) . ']';
                        else
                            $inner = Expression::quote($part['field'] . ' ' . strtoupper($op) . ' ' . $part['value']) . '';
                    } else {
                        ...
                        if ($part['value'] == 'null') {
                            if ($op == 'is not') {
                                ...
                                $fnParts[] = 'null';
                            $inner = Expression::quote($part['field']);
                        } else
                            $inner = Expression::quote($part['field']) . ', ' . Expression::quote(strtoupper($op)) . ', ' . $this->wrapValue($part['value']);
                ...
                case CriterionTypes::IN_FIELD: // for sub queries
                    ...
                    if ($part['raw_field'] || ($part['raw_value'] && !$valuePHPArr)) {
                        // Raw Methods
                        $fn = $this->getValue($part['sep']) == 'or' ? 'orWhereRaw' : 'whereRaw';
                        $queryVal .= '->' . $fn . '(' . Expression::quote($part['field'] . ' ' . strtoupper(implode(' ', $part['operators'])) . ' ' . $part['value']) . ')';
                    } else {
                        ...
                        $queryVal .= '->' . $fn . '(' . Expression::quote($part['field']) . ', ';
                        if ($valuePHPArr) {
                ...
                case CriterionTypes::AGAINST:
                    $fn = $this->getValue($part['sep']) == 'or' ? 'orWhereRaw' : 'whereRaw';
                    $queryVal .= '->' . $fn . '(' . Expression::quote($part['field'] . ' AGAINST ' . $part['value']) . ')';
                    break;
                case CriterionTypes::FN:
                    $fn = $this->getValue($part['sep']) == 'or' ? 'orWhere' : 'where';
                    $fn = $this->fnMerger(array($fn, $part['fn']));
                    $op = $part['operator'];
                    $inner = Expression::quote($part['field']) . ', ' . Expression::quote(strtoupper($op)) . ', ' . $this->wrapValue($part['value']['value']);
                    $queryVal .= '->' . $fn . '(' . $inner . ')';
                    break;
                default:
                    break;
    ...
    private function buildBetween($part)
    {
        ...
        if (in_array('not', $part['operators'])) { // is not between?
            ...
            $fn = $this->fnMerger($fnParts);
            $query .= $fn . '(' . $this->builder->build($part['field'], $part['raw_field']) . ', ';
        ...
        $query .= '[' . $this->builder->build($part['value1'], $part['raw_values'][0]) . ', ' . $this->builder->build($part['value2'], $part['raw_values'][1]) . ']' . ')';
